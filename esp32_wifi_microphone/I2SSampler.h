...
protected:
    void addSample(int32_t sample, uint8_t bps);
    virtual void configureI2S() = 0;
    virtual void processI2SData(uint8_t *i2sData, size_t bytesRead, uint8_t m_bps) = 0;
...
public:
   ...
   // uint8_t m_bps;
    ...
    void start(i2s_port_t i2sPort, i2s_config_t &i2sConfig, uint8_t bps, int32_t bufferSizeInSamples, TaskHandle_t writerTaskHandle);
