...
int I2SMEMSSampler::read(int16_t *samples, int count, uint8_t bps)
{
    ...
    {
        size_t bytes_read = 0;
        i2s_read(m_i2sPort, (void *)raw_samples, sizeof(int32_t) * std::min(count, 256), &bytes_read, portMAX_DELAY/8);
        int samples_read = bytes_read / sizeof(int32_t);
        for (int i = 0; i < samples_read; i++)
        {
            if (bps  == 16) samples[sample_index] = ((raw_samples[i]) >> 13) >>signal_gain;
              else if (bps  == 24) {
