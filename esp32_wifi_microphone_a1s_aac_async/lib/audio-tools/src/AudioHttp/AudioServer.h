...
class AudioEncoderServer  : public AudioServer {
    ...
    public:
         ...
         * @param channels 
         */
        void begin(Stream &in, int sample_rate, int channels, int bits_per_sample=32, BaseConverter<int16_t> *converter=nullptr) {
	 		LOGD(LOG_METHOD);
            this->in = &in;
 ...
 * with a callback method or from a Stream.
 */
class AudioAACServer : public AudioEncoderServer {
    public:
        /**
         * @brief Construct a new Audio W A V Server object
         * We assume that the WiFi is already connected
         */
        AudioAACServer() : AudioEncoderServer(new AACEncoderFDK()){
        }
         ...
         * @param password 
         */
        AudioAACServer(const char* network, const char *password) : AudioEncoderServer(new AACEncoderFDK(), network, password) {
        }
